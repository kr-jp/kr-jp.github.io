...
!!! warning "日本語に`\w`は使えない"
    `\w`は英数字とアンダースコアにしかマッチしません。日本語の文字を検索するには、`[一-龠ぁ-んァ-ヴー]+`のように明示的に指定する必要があります。

## 改行の仕組み

テキスト資料を収集してコーパスを構築するときには、一行に一つの文があった方が処理をするうえではいいです。たとえば、「正規表現」という文字列が入っている例文を収集する場面を考えてみましょう。以下の例では、一つの行に複数の文があって、しかも、すべての文に「正規表現」という文字列が入っています。
...
この文章を、一行に一つの文のみが残るようにしたい。つまり、以下のような文にしたいと想定してみましょう。
...
日本語の場合、必ずではありませんが、文の終わりには「。」や「．」が来ます。この句点を頼りに改行をすれば、一行一文にすることができるでしょう。改行は、正規表現で次のように表します。
...
Wordを使うとき、「⏎」の形をしている記号を見たこと、ありませんか？

<figure markdown style="text-align: center;">
![改行記号](../assets/img/special-lecture-on-linguistics/07a-typewriter.png){: style="width: 65%;"}
<figcaption>図1: Microsoft Wordで改行を表す記号</figcaption>
</figure>

この記号は「←」と「↓」を組み合わせたものです。これは、かつての[テレタイプ](https://ja.wikipedia.org/wiki/%E3%83%86%E3%83%AC%E3%82%BF%E3%82%A4%E3%83%97%E7%AB%AF%E6%9C%AB)（teletype）という通信機器に由来しています。テレタイプは、電信でテキストを送受信できる機械で、1960年代のコンピュータの入出力装置として広く使われていました。キーボードで文字を打つと、電気信号として送信され、受信側で自動的に印字されます。

テレタイプで文字を打つとき、行末に到達すると、次の行に移るには2つの動作が必要でした。

1. CR（Carriage Return）：印字ヘッドを行の先頭に戻す
2. LF（Line Feed）：紙を1行分送る

<!-- ここで重要なのは、なぜ`\r\n`（CR→LF）の順番なのかという点です。

印字ヘッドを右端から左端に戻す動作（CR）には、物理的に時間がかかりました。一方、紙を1行送る動作（LF）は高速で完了します。もしLFを先に送り、CRが完了する前に次の文字が送られてくると、印字ヘッドが移動中に文字が印字され、ページの途中で文字がにじんでしまいます。

そこで、まずCR（時間のかかる動作）を開始し、その移動時間中にLFを実行するという工夫がなされました。これにより、両方の動作が完了してから次の文字を受け取れるようになったのです。 -->

この規約は、現在でもWindowsのテキストファイルで`\r\n`として使われています。一方、UnixやmacOSでは`\n`が採用されています。

!!! tip "タイプライターでの改行"
    タイプライターで改行がどのように行われているのかを見ると、上での説明とは反対に、LFの後にCRが行われます。正規表現で言うと、`\r\n`ではなく、`\n\r`の順番となっています。[人間がタイプライターを手動で打つ動画](https://youtu.be/8cxJm3jcESE)と、[テレタイプが自動で文字を打つ動作](https://youtu.be/S81GyMKH7zw?t=246)を見ると、その違いがわかるでしょう。
...
!!! tip "色々なテキストエディタ"
    もし、すでに[サクラエディタ](https://sakura-editor.github.io/)や[秀丸エディタ](https://hide.maruo.co.jp/software/hidemaru.html)のようなテキストエディタを使っているのであれば、それを使っても構いません。その他のテキストエディタでもいいですが、grep（global regular expression print）機能を使いますので、この機能が搭載されているのかを確認してみましょう。
...
**Visual Studio Code**（以下VS Code）」は、主にプログラミングをするときに使うコードエディタですが、テキストエディタとして使うこともできます。

!!! example "メモ帳、さようなら"
    
    === "① ダウンロード"
        [Visual Studio Code](https://code.visualstudio.com/)のウェブサイトに接続して、VS Codeをダウンロードし、インストールをしましょう。
    
    === "② 日本語環境に変更"
        VS Codeを立ち上げて、日本語環境にしてみましょう。
        
        自分のパソコンが日本語環境の場合、「言語パック」のインストールについて聞かれることがあります。その場合は「インストールして再起動」を押しましょう。

        もし、そのようなメッセージは出ない場合は、VS Codeの左側にある「Extentions」ボタンを押して、検索窓に「Japanese」を入力します。一番上にある「Japanese Language Pack for Visual Studio Code」をインストールしましょう。
    ...
    === "③ Hello, World!"
        ...
        `Ctrl+F`を押して文字列「Hello」を正規表現で検索してみましょう。正規表現を使うには、`.*`ボタン（正規表現を使用する）ボタンを押します。
        
        <figure markdown style="text-align: center;">
        ![改行記号](../assets/img/special-lecture-on-linguistics/07a-vscode_find.png){: style="width: 60%;"}
        <figcaption>図2: 検索窓を開いて正規表現の利用を有効にする</figcaption>
        </figure>

        

        正規表現が問題なく動作するのかを、確認してみましょう。

### 改行を入れてみる

!!! example "正規表現の力"
    以下の文字列をテキストエディタに張り付けましょう。その後、正規表現を利用して改行を入れ、一行一文にしてみましょう。

    ``` { .text .copy title="文字列" }
    正規表現を学ぶと、私にとってどんないいことがあるのだろうか。他の事務職員がテキストファイルの修正のために、いちいち手作業で、あるいは、数十回の置換をしなければならないところを、私は正規表現を利用して一発で作業を終えることができる（かもしれない）。ありがとう、正規表現。
    ```
    
    今回は`Ctrl+H`を押して（あるいは検索窓の左端にある`>`を押して）、検索のところに「。」を、「置換」には「。\n」と入力して「置換」ボタン、あるいは、`Enter`を押しましょう。改行は`\r\n`と説明したのですが、VS Codeでは、OSがWindowsでも`\n`で改行を表すことができます。「すべて置換」ボタンを押して、一括で置き換えることもできます。

!!! tip "行の折り返し"
    一行の文字列が長い場合は、すべての文字列が画面に収まらない場合があります。

    <figure markdown style="text-align: center;">
    ![改行記号](../assets/img/special-lecture-on-linguistics/07a-vscode_unwrap.png){: style="width: 60%;"}
    <figcaption>図3: 小さい画面だと文字列が収まらない</figcaption>
    </figure>

    そのときには、メニューにある`表示`から`右端での折り返し`を選択すると、すべての文字列が画面の大きさに合わせて表示されるようになります。

    <figure markdown style="text-align: center;">
    ![改行記号](../assets/img/special-lecture-on-linguistics/07a-vscode_wrap.png){: style="width: 60%;"}
    <figcaption>図4: 文字列全体が画面に収まる</figcaption>
    </figure>