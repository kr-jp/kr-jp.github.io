...
!!! example "下ごしらえ"
    ...
    === "① 作品をダウンロード"
        ...
        !!! warning "解凍には時間がかかる"
            Macの場合はすぐ解凍ができますが、Windowsの場合、「すべて展開」をすると5〜10分程度かかることがあります。こういうときには、[7-Zip](https://7-zip.opensource.jp/)をインストールして解凍するといいでしょう。インストールをした場合には、以下の手順で、7-Zipで解凍することができます。

            <figure markdown style="text-align: center;">
            ![改行記号](../assets/img/special-lecture-on-linguistics/09-7-zip-a.png){: style="width: 65%;"}
            <figcaption>図2: ファイルを右クリックして「その他…」を選択</figcaption>
            </figure>

            <figure markdown style="text-align: center;">
            ![改行記号](../assets/img/special-lecture-on-linguistics/09-7-zip-b.png){: style="width: 65%;"}
            <figcaption>図3: 「7-Zip」に入って「…展開」を選択</figcaption>
            </figure>
    ...
    === "④　ファイルの除外"
        ...
        <figcaption>図5: 除外するファイルの指定</figcaption>
        </figure>

        図5のように「除外するファイル」に、以下の文字列を入力します。このようにすると、すべてのフォルダの中から（`**/`）、ファイル名に`_txt`という文字列が含まれているものは（`*_txt*`）、検索の対象から除外します。
...
!!! tip "最大検索件数の引き上げ"
    ...
    ちゃんと20,000以上の検索結果が出るのか、grep機能を使って試してみましょう。grepで100万件以上を表示させたい場合には、テキストエディタではなく、[__10. Colabの活用—基礎編__](10a-colab-basics.md)で触れるPythonを利用した方がいいでしょう。
...
<figcaption>図6: コンテキスト行の切り替え</figcaption>
</figure>
...
!!! example "切り取り"
    ...
    1. 検索結果が表示されているところにカーソルを置き、`Ctrl + Shift + L`（Mac：`command + Shift + L`）を押しましょう。すると、正規表現にマッチした文字列だけが選択されます。
...
まずは、[__4. 形態素と形態素解析__](04-pos.md)での記憶を呼び戻して、Web茶まめを使って形態素解析をやってみましょう。