...
!!! example "Colabにインストール"
    ...
    === "① インストールコマンド"
        ...
        ```python { .text .copy title="fugashiとUniDicをインストール" }
    ...
    === "② インストール確認"
        ...
        ```python { .text .copy title="fugashiの動作確認" }
...
!!! example "フィクションではありません"
    ...
    === "③ コードを翻訳してもらう"
        コードに簡単な注釈が書いてありますが、コードを書いたことのない人からすると、どういう意味なのか、しっくりこないかもしれません。このコードをAIに渡して、さらにわかりやすく説明してもらうこともできます。以下は、Gemini 3 Proが生成したものです。
...
!!! example "AIにコードを書いてもらおう"
    ...
    - 使用するAI: [ChatGPT](https://chat.openai.com/)、[Claude](https://claude.ai/)、[Gemini](https://gemini.google.com/)のいずれか
    - 分析対象: 配布するテキストファイル（AIが作成中なので今日中に配布する）
    - 締め切り: 1月22日（木）
    
    課題内容:
    
    以下の3つのタスクから2つを選び、AIの力を借りてコードを作成・実行してください。
    
    === "① 形容詞の抽出"
        
        目標: テキスト中の形容詞だけを抽出して、頻度順に表示する
    ...
    === "② 文の長さの平均"
        ...
        目標: 文の長さ（形態素数）の平均を計算する
        
        プロンプト例:
        
        ``` { .text .copy title="プロンプト" }
        Google Colabで、fugashiを使って日本語テキストの各文の
        ...
        - テキストは変数textに入っています
        - 文の区切りは「。」「！」「？」とします
        - 結果は「文の数」「平均形態素数」を表示してください
        ```
        
    === "③ 助詞の比較"
        
        目標: 助詞「は」と「が」の出現回数を比較する
        
        プロンプト例:
        
        ``` { .text .copy title="プロンプト" }
        Google Colabで、fugashiを使って日本語テキスト中の
