...
| ワイルドカード対応 | 柔軟な文字列検索（詳しくは[__7. 正規表現__](07-regular-expressions.md)で学習） | 「〜たり」で終わる動詞をすべて検索 |
...
### 不連続的なコロケーション
コロケーションには、上記のような連続的なコロケーションだけでなく、不連続的なコロケーション（discontinuous collocation）もあります。たとえば、「どうりで～わけだ」「道理で～はずだ」といった例が挙げられます。このコロケーションの実例としては、次のような例が挙げられます。
...
ところで、話し言葉では「道理」は「どうり」ではなく、「どおり」と言う例も見られています。

> - どおりで鼻が効くわけだな。（『深夜食堂3（シーズン1/エピソード3）』）
 - あ、何だ、向こう岸か。どおりで見つかんないわけだ。（『孤独のグルメ（シーズン5/エピソード11）』）
 - この腹騒ぎは…。5時か。どおりで、腹が…減るわけだ。（『孤独のグルメ（シーズン9/エピソード1）』）

「中納言」で提供している話し言葉コーパスにおいて、このような例は見つかりませんでした。「コーパスにはないけれども、現実世界ではありうる」という点は、コーパスを使ううえで、注意すべき点であります。この点からすると、コーパスを利用するときには、自分が指定した検索で「どのような例が含まれないのか」を考えることが重要になります。たとえば、「どうりで～わけだ」「道理で～はずだ」のようなコロケーションでは、文末の「わけだ」「はずだ」が現れない場合があります。

> - へえ～！どうりでね。あか抜けでるど思ったぁ。（『深夜食堂3（シーズン1/エピソード6）』）
- どうりでシュッとしとってやなぁと思いました。東京やって。（『孤独のグルメ（2021大晦日スペシャル〜激走！絶景絶品・年忘れロードムービー〜）』）

## 検索履歴の活用
中納言には、過去の検索条件を保存して、後で再利用できる「検索履歴」機能があります。

!!! example "検索履歴を使いこなす"
    最初に、中納言のトップページから「履歴で検索」をクリックすると、過去に実行した検索の一覧が表示されます。
    
    そこから、再度実行したい検索履歴を確認して、「編集して検索」をクリックすると「検索条件式」が出てきます。その状態で「検索」ボタンを押すと、同じ条件で再検索が実行されます。
...
!!! example "オノマトペの分布を調べる"
    ...
    2. 各コーパスでの出現頻度を記録
    3. 以下の点について考察：
        - 書き言葉（BCCWJ）と話し言葉（CSJ、CEJC）で違いはあるのか？
...
!!! example "興味のある表現を徹底的に調べる"
    ...
    1. 前回の課題で選んだ言葉を、中納言で検索
    2. 以下の点を調べる：
        - 語彙素と書字形出現形で、どのくらい結果が違うか？
        - 短単位と長単位で、どのような違いがあるか？
        - 前後に共起する語のパターンは？
